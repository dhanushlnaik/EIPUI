...
export async function GET(request: Request, context: Ctx) {
  ...
  try {
    ...
    const getTimelineRankings = async (
      typeFilter?: string,
      repoFilter?: string,
      sortBy?: string
    ) => {
      ...
      return [
        results.map((c: any, index: number) =>
          transformContributor(c, skip + index + 1)
        ),
        total,
      ] as const;
    };
    ...
    switch (mode) {
      case "overall":
        ...
        if (isTimePeriod) {
          [ranking, total] = await getTimelineRankings(undefined, repo, "totalCommits");
        } else {
      ...
      case "commits":
        ...
        if (isTimePeriod) {
          [ranking, total] = await getTimelineRankings("commit", repo, "totalCommits");
        } else {
          [ranking, total] = await Promise.all([
            contributors
              .find(filter)
      ...
      case "prs":
        ...
        if (isTimePeriod) {
          [ranking, total] = await getTimelineRankings("pr", repo, "totalPRs");
        } else {
          [ranking, total] = await Promise.all([
            contributors
              .find(filter)
      ...
      case "reviews":
        ...
        if (isTimePeriod) {
          [ranking, total] = await getTimelineRankings("review", repo, "totalReviews");
        } else {
          [ranking, total] = await Promise.all([
            contributors
              .find(filter)
      ...
      case "comments":
        ...
        if (isTimePeriod) {
          [ranking, total] = await getTimelineRankings("comment", repo, "totalComments");
        } else {
          [ranking, total] = await Promise.all([
            contributors
              .find(filter)
      ...
      case "issues":
        ...
        if (isTimePeriod) {
          [ranking, total] = await getTimelineRankings("issue", repo, "totalIssues");
        } else {
          [ranking, total] = await Promise.all([
            contributors
              .find(filter)
      ...
      case "monthly":
        ...
        description =
          `Top contributors in the past ${mode === "weekly" ? "week" : "month"}`;
        [ranking, total] = await getTimelineRankings(undefined, repo, "totalCommits");
        break;
